...
namespace DAO
{
    public class DAO_KhachHang
    {
        ...
        public DataTable loadNguoiChamSoc()
        {
            DataTable dt = new DataTable();
            try
            {
                conn.Open();
                string sql = "SELECT * FROM dbo.NGUOICHAMSOC";
        ...
        public DataTable GetBe()
        {
            DataTable dt = new DataTable();
            try
            {
                conn.Open();
                string sql = "SELECT * FROM dbo.THONGTINBE";
                SqlCommand cmd = new SqlCommand(sql, conn);
                SqlDataAdapter da = new SqlDataAdapter(cmd);
                da.Fill(dt);
                return dt;
        ...
        public DataSet GetNCS()
        {                   
            //try 
            //{
                conn.Open();
                DataTable dtNCS = new DataTable();
                string sqlNCS = "SELECT * FROM dbo.NGUOICHAMSOC";
                SqlCommand cmdNCS = new SqlCommand(sqlNCS, conn);
                SqlDataAdapter daNCS = new SqlDataAdapter(cmdNCS);
                daNCS.Fill(dtNCS);
                DataTable dtBe = new DataTable();
                string sqlBe = "SELECT * FROM dbo.THONGTINBE";
                SqlCommand cmdBe = new SqlCommand(sqlBe, conn);
                SqlDataAdapter daBe = new SqlDataAdapter(cmdBe);
                daBe.Fill(dtBe);

            DataSet dts = new DataSet();
            dts.Tables.Add(dtNCS);
            dts.Tables.Add(dtBe);
            dts.Relations.Add(new DataRelation("Danh sách bé", dtNCS.Columns[0], dtBe.Columns[5]));
            //}
            //catch { }
            //finally { conn.Close(); }
            conn.Close();
